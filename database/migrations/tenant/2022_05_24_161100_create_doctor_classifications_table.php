...
class CreateDoctorClassificationsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('doctor_classifications', function (Blueprint $table) {
            $table->id();
            ...
            $table->tinyInteger('synchronized')->default(1)->nullable();
            $table->timestamp('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));
            $table->timestamp('updated_at')->nullable()->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
            $table->foreign('doc_id')->references('id')->on('doctors');
            $table->foreign('cf_id')->references('id')->on('classifications');
    ...
    {
        Schema::dropIfExists('doctor_classifications');
    }
}
