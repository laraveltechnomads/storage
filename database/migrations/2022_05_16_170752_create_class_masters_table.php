...
class CreateClassMastersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('class_masters', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('class_unit_id')->nullable();
            $table->string('c_code',20)->nullable();
            $table->string('deposit_for_ipd',20)->nullable();
            $table->string('deposit_for_ot',20)->nullable();
            ...
            $table->foreign('class_unit_id')->references('id')->on('units');
