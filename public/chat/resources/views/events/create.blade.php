...
@section('breadcrumb-title')
    <h1>Add Event</h1>
    @if($login == 'ADM')
      <a href="{{ url()->previous() }}" tooltip="Blogs List" class="badge badge-primary"><i class="fa fa-arrow-left"></i></a>
    @else
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                ...
                <form class="forms-sample" id="blogForm" action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  ...
                  <div class="row">
                    <div class="col-sm-12">
                      ...
                      <div class="form-group">
                        <label for="title">{{ __('Church Name')}}</label>
                        <input type="text" class="form-control" value="{{ $church->church_name }}" readonly>
                      </div>
                      <div class="form-group">
                        <label for="title">{{ __('Event Name')}}</label>
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" placeholder="Title" value="{{old('title')}}" required="" autocomplete="off">
                        @error('title')
                      ...
                      <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" placeholder="Description" required="" autocomplete="off">{{old('description')}}</textarea>
                        @error('description')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                      </div>
